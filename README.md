# Привет, я Михаил 👋

Студент РУТ МИИТ (ЦИТП) | Python-разработчик | Аналитик данных

Ищу стажировку в направлении анализа данных или backend-разработки.
Работаю с реальными отраслевыми данными — транспорт, РЖД, климатология.

---

## Стек

![Python](https://img.shields.io/badge/Python-3776AB?style=flat&logo=python&logoColor=white)
![Django](https://img.shields.io/badge/Django-092E20?style=flat&logo=django&logoColor=white)
![pandas](https://img.shields.io/badge/pandas-150458?style=flat&logo=pandas&logoColor=white)
![PostgreSQL](https://img.shields.io/badge/PostgreSQL-336791?style=flat&logo=postgresql&logoColor=white)
![MongoDB](https://img.shields.io/badge/MongoDB-47A248?style=flat&logo=mongodb&logoColor=white)
![Playwright](https://img.shields.io/badge/Playwright-45ba4b?style=flat&logo=playwright&logoColor=white)
![Jupyter](https://img.shields.io/badge/Jupyter-F37626?style=flat&logo=jupyter&logoColor=white)
![scikit-learn](https://img.shields.io/badge/scikit--learn-F7931E?style=flat&logo=scikit-learn&logoColor=white)

---

## Проекты

| Проект | Описание | Стек |
|--------|---------|------|
| [СберТройка \| Антифрод](https://github.com/MishanyaKudesnik/sbertroika-antifraud) | Выявление мошенничества в 490k транзакций. 6 845 подозрительных карт (8.3%) | Django, pandas |
| [Ловец Аномалий \| РЖД](https://github.com/MishanyaKudesnik/wheelset-anomaly-detector) | Классификатор аномалий колёсных пар. 1.49 млн строк, 6 категорий | pandas, seaborn |
| [Auto.ru Parser + Bot](https://github.com/MishanyaKudesnik/autoru-parser-bot) | Парсинг auto.ru, статистический анализ, Telegram-бот с PDF-отчётами | Playwright, pandas, scipy |
| [Таксопарк](https://github.com/MishanyaKudesnik/taxi-park-management) | Django веб-приложение управления таксопарком. CRUD, дашборд | Django, SQLite |
| [Атмосфера и Океан](https://github.com/MishanyaKudesnik/atmosphere-ocean-analysis) | EDA климатических данных NASA/NOAA | pandas, polars, plotly |
| [SQL Projects](https://github.com/MishanyaKudesnik/sql-projects) | PostgreSQL CRUD + агрегация MongoDB | psycopg2, pymongo |
| [Mobile Price Classification](https://github.com/MishanyaKudesnik/mobile-price-classification) | ML-классификация цен на телефоны. Accuracy 96.5%, 3 модели сравнения | scikit-learn, pandas |

---

## Контакты

mischukow1903@gmail.com  
https://t.me/kudesnik_mm
